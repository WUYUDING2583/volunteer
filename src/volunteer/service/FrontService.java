...
import volunteer.dao.IFrontDAO;
import volunteer.config.*;
...
public class FrontService implements IFrontService{
	...
	private IFrontDAO dao ;

	public IFrontDAO getDao() {
		return dao;
	}

	public void setDao(FrontDAO dao) {
		this.dao = dao;
	}
	...
	public String getActivityDetail(ViewApplyPK pk) {
		...
		try {
			...
			if(endtime.getTime()>now.getTime())System.out.println(time);
			if (time > 0) {
