...
public class ActivityDAO extends BaseHibernateDAO implements IActivityDAO{
	...
	public String addActivity(ActInfo info)
	{
		...
		session.flush();
		tran.commit();System.out.println("添加活动成功："+result);
		session.close();
		return result;
	}
	//获取该组织的所有活动信息
	...
	public String deleteActivity(String Ano)
	{
		...
		try
		{
			Session session=getSession();
			Transaction tran=session.beginTransaction();
			ActInfo temp=(ActInfo) session.get(ActInfo.class, Ano);System.out.println("根据活动编号删除活动1");
	...
	public List endAndUp(String college)
	{
		...
		session.clear();
		session.close();
		System.out.println("FFFJKKHFFGH");
		return result;
	}
