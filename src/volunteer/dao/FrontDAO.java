...
public class FrontDAO extends BaseHibernateDAO implements IFrontDAO{
...
//获取活动时间段
	...
	public ViewApply viewApply(ViewApplyPK pk) 
	{
		...
		*/
		...
		System.out.print("PK:"+pk.getAno());
		System.out.print("PK:"+pk.getAdeadline());
		System.out.print("PK:"+pk.getAname());
		if(result==null)System.out.println("DSFSFSSF");
		tran.commit();
...
//获取用户信息
	public List getUserInfo(String No) 
	{
		...
		Query queryObject = session.createQuery(queryString);
		*/
		User temp=(User) session.get(User.class, No);
		
		//List result=queryObject.list();
		List<User> result=new ArrayList<User>();
		result.add(temp);
		System.out.println("USERMESS:"+temp.getName());
		tran.commit();
		session.flush();
		session.close();
		return result;
